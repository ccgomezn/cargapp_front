...
import { deleteCity } from '../../../helpers/api/adminCalls';

const deleteFunction = (id) => {
  return function () {
    (deleteCity(id)
      .then((response) => {
        window.location.href = window.location.protocol + '//' + window.location.host + '/dashboard/admin/cities/';
  ...
  switch (type) {
    ...
    case 'MultipleButtonCell':
      ...
      var function1 = function () {
        window.location.href = window.location.protocol + '//' + window.location.host + '/dashboard/admin/cities/edit/' + object['id'];
      }
...
const columns = [
  ...
  {
    title: <IntlMessages id="antTable.title.state_location" />,
    key: 'state',
    width: '12%',
